...
public class StringItemDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        var idRect = new Rect(position.x, position.y, position.width / 3, position.height);
        var dataRect = new Rect(position.x + (position.width / 3) + 5, position.y, position.width / 3 * 2, position.height);
